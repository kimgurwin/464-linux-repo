...
'''
Description:
This file allows for the control of a robot that shuffles for locomotion. It periodically moves up to 2 motors at defined intervals depending on the keys pressed, allowing the user to control the robot using a pygame window.

Controls:
- W: Move both modules forward/backward
- A: Move left module
- D: Move right module
...
class WalkerApp( JoyApp ):
    ...
    def __init__(self,*arg,**kw):
        ...
        JoyApp.__init__(self,robot=dict(count=2),*arg,**kw)
    ...
    def onStart(self):
        ...
        self.robot.at.Nx45.set_pos(3000)
        self.robot.at.Nx05.set_pos(3000)
          
    def onStop( self ):
      # Make sure we go_slack on all servo modules when we exit
      """self.robot.at.Nx0Fw.set_pos(-5000)"""
      self.robot.at.Nx45.set_pos(3000)
      self.robot.at.Nx05.set_pos(3000)
      self.robot.off()
